...
# Step back one revision at a time and STOP AT THE BASELINE.  If any downgrade
# fails, the trap fires + cleanup + exit 1.
#
# The loop used to continue past the baseline to `base`, which contradicted
# this script's own docstring and could never pass: the baseline's downgrade is
# autogenerated "drop the entire schema", and it has drifted from the objects
# that actually exist (it drops ix_security_findings_id, whose table a later
# revision renamed).  Nobody rolls back below the baseline anyway — that isn't
# a rollback, it's an empty database, and the recovery path there is the
# pre-deploy dump.  Every revision whose downgrade an operator could realistically
# need is still exercised.
...
while true; do
    ...
    if [[ "${current}" == "${BASELINE_REVISION}" ]]; then
        echo "  reached baseline ${BASELINE_REVISION} — stopping (see comment above)"
        break
    fi
