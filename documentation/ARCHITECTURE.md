...
3. **Per-entry: sanity check → approval → results.** For each entry, the agent MUST first run a host sanity check (banner grab, DNS lookup, ICMP) and POST it to `/sanity-check`. The UI gates test execution on a passing sanity check. Then for each test the agent PRESENTS the command and applies **approve-by-exception** (v2.279.0, adopted after testing and evaluation): a command that is in policy — an approved tool, against a host already in the inventory, writing into the session's working directory — runs and is announced; anything else waits for the operator's explicit approval. Results are POSTed to `/test-results`. BlueStick cannot enforce that boundary (commands run on the operator's machine; the client's sandbox is what holds), which is exactly what `GET /.well-known/networkmapper.json` now publishes: `command_approval: "by_exception"`, `command_approval_enforced_by: "agent_and_client_sandbox"`, beside the properties the server DOES enforce (it executes nothing; execution needs a human-approved plan; a key's authority is its operator's project role). Status transitions on each entry track progress.
