...
> **Last verified against:** backend 2.343.3 / frontend 5.219.2 (2026-09-18)
...
| NetExec (NXC) | `.json`, `.txt` | `backend/app/parsers/netexec_parser.py` | Text output: the SMB and LDAP enumeration lines (SMB signing posture is recorded from the SMB line). JSON: `spider_plus` module output. Put `netexec` or `nxc` in the filename for explicit routing. |
