...
| RDAP | `.json`, `.ndjson` | `backend/app/parsers/rdap_parser.py` | Network-registration provenance — who a netblock is registered to (org, country, registry handle) and, where the source supplies it, the ASN. Generate with the bundled `./scripts/rdap-lookup.py` (stdlib only, no pip install), which resolves the authoritative RDAP server per prefix via the IANA bootstrap registry, deduplicates **per netblock** rather than per host, and skips private/reserved space. Example: `./scripts/rdap-lookup.py --input hosts.csv --output rdap.ndjson`. Ingest attaches each host to the block covering it, queryable as `org:` / `asn:` / `cloud:` in the host DSL. Egress deliberately stays operator-side — the backend makes no registry calls, so air-gapped deployments work and registry rate limits never touch a web request. |
