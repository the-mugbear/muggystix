 ...
 * that pertain to this host.  Records are grouped by ``record_type``
 * (A / AAAA / PTR / CNAME / MX / NS / TXT / SOA) and each row's
 * ``resolver_name`` is rendered as a badge so the operator can see
 * which resolver produced each answer.
 *
 * These records come only from UPLOADS (dnsx JSON, DNS CSV, amass) — BlueStick
 * never originates DNS queries itself. Run your lookups on your own host and
 * upload the output; there is no server-side "resolve now".
 *
 * Lazy-loaded — fetches on mount.  Renders nothing when the project has no DNS
 * records at all, to avoid empty-card noise on freshly-discovered hosts.
 */
