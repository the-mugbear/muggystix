...
4. `frontend/package-lock.json` — its two `version` fields (`npm install --package-lock-only`
   after bumping; the image build's `npm ci` fails when the lock disagrees with `package.json`)
...
- **Authoring a migration.** Revisions here are mostly hand-written (copy the newest file in
  ...
  `alembic check` proposes dropping its tables); write a real `downgrade()` (the round-trip
  script runs it). A DATA migration is applied to your dev database the moment anything imports the
