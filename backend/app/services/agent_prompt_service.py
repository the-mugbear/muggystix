...
def build_session_instructions(
    *,
    ...
    instructions = (
        ...
        f"output file as it finishes; do not hold results for one upload at the "
        f"end.\n\n"
        f"### Last — end the session (MANDATORY)\n"
        f"When the operator says you are done, or you have nothing left to do: "
        f"close every open phase (`POST /agent/recon/complete`, "
        f"`POST /agent/execution-sessions/{{id}}/complete`), submit feedback "
        f"(below), then `POST {base_url}/agent/session/end` (MCP `end_session`) "
