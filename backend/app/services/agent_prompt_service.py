...
def build_assist_instructions(
    *,
    ...
    instructions = (
        ...
        f"findings behind those counts — severity, CVE/plugin id, affected "
        f"port, exploitability, description, remediation (`solution`), and "
        f"scanner `evidence`. Filter `?severity=critical,high`; paginated with "
        f"`total`/`has_more`. Use this for evidence-rich reporting rather than "
        f"citing bare counts.\n"
